    ...
    public class AdvancedDatabaseCrawler : DatabaseCrawler
    {
        ...
        protected virtual List<SCField> FilteredFields(Item item)
        {
            ...
            if (IndexAllFields)
            {
                ...
                //<!--REMOVED FOR FALLBACK DEMO-->
                //filteredFields.AddRange(item.Fields);

                //<!--ADDED FOR FALLBACK DEMO-->
                //filterFields.AddRange(item.Fields) was used to add fields to the filteredField List.
                //when the item was published the field values were coming blank for most of the fields for fallback languages 
                //(i.e. en-US and re-EU)

                //changed the logic to loop through the fields and if field value is null or empty, 
                //then get the value from its fall back language and do this recursively until the Global language
                //Add that key/value pair to the filteredFields list.
            ...
            else if (HasFieldIncludes)
            {
                foreach (var includeFieldId in from p in FieldFilter where p.Value select p)
                {
                    ...
                    //<!--ADDED FOR FALLBACK DEMO-->
                    var field = item.Fields[ID.Parse(includeFieldId.Key)];
                    var currentItem = item;
                    currentItem = CrawlerHelper.GetSitecoreFallbackItem(item, field);
                    if (currentItem.Fields[field.ID] != null)
                        filteredFields.Add(currentItem.Fields[field.ID]);
