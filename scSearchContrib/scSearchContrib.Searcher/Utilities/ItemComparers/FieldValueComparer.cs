...
namespace Sitecore.SharedSource.Data.Comparers.ItemComparers
{
	  ...
	  System.Collections.Generic.IComparer<Item>
	{
		...
		/// </returns>
		public int CompareFieldValues(
		  Sitecore.Data.Fields.Field firstField,
		  Sitecore.Data.Fields.Field secondField)
		{
			...
			{
				DateTime firstDate = Sitecore.DateUtil.IsoDateToDateTime(firstField.Value);
				DateTime secondDate = Sitecore.DateUtil.IsoDateToDateTime(secondField.Value);
		  ...
		  Sitecore.Data.Items.Item secondItem)
		{
			...
			if (String.IsNullOrEmpty(secondField.Value)
			  && !String.IsNullOrEmpty(firstField.Value))
			{
				return (int)SortState.Correct;
			}

			if (String.IsNullOrEmpty(firstField.Value)
			  && !String.IsNullOrEmpty(secondField.Value))
			{
				return (int)SortState.Reverse;
			}
		  ...
		  Sitecore.Data.Fields.Field secondField)
		{
			...
			if (typeName == "integer"
			  || firstField.ID == Sitecore.FieldIDs.Sortorder
			  || (Int32.TryParse(firstField.Value, out test)
			  && Int32.TryParse(secondField.Value, out test)))
			{
			...
			if (typeName == "date"
			  || typeName == "datetime"
			  || (Sitecore.DateUtil.IsIsoDate(firstField.Value)
			  && Sitecore.DateUtil.IsIsoDate(secondField.Value)))
			{
